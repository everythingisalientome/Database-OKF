...
| 2 | Crawler skeleton, Tier A only (A1–A6, A5), allow-list; adapters: PostgreSQL, SQL Server, Teradata (UNVERIFIED) | Acceptance vs live Chinook on PG + MSSQL containers; allow-list rejection test | NEXT |
| 3 | Measuring pass: stats-first A6, B1/B2 batched profiles + derived rates/flags, B3 top-N, C1 bottom-k + keyed hashing, gates & budgets | Measured numbers == fixture numbers, digit-for-digit | |
| 4 | LLM annotation pass + OKF bundle emission (descriptions at DB/table/column level, index.md) | Emitted bundle passes session-1 validator; annotator sees derived artifacts only | |
